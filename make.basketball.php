...
class NewBasketballStatus extends NewData
{
	public $_sport;
	public $_type;
	public $match_id;
...
//
function basketball_fixtures($xml) {
	$err = array();
	foreach($xml->competition as $comp) {
		...
		foreach($comp->match as $k=>$match) {
			...
			$t->club_id1 = $match->attributes()->club1ID;
			$t->club_id2 = $match->attributes()->club2ID;
